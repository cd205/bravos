...
must_haves:
  - position() callback appends STK positions to _positions (non-STK ignored)
  - positionEnd() sets _positions_done event
  - openOrder() callback appends to _open_orders
  - openOrderEnd() sets _orders_done event
  - accountSummary() stores tag values in _account_summary dict
  - accountSummaryEnd() sets _summary_done event
  - run_startup_reconciliation() calls reqPositions, reqAllOpenOrders, reqAccountSummary then waits
  - _write_position_snapshot() inserts rows into broker_positions_snapshot
  - _reconcile_against_db() logs WARNING on mismatch, never writes to position_lots
---
