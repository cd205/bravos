...
must_haves:
  - start_heartbeat_monitor() starts a daemon thread named ibkr-heartbeat
  - _heartbeat_loop() calls reqCurrentTime every 60s and triggers reconnect on 10s timeout
  - error(504) triggers _trigger_reconnect immediately
  - error(1100) triggers _trigger_reconnect immediately
  - error(2104) does NOT trigger reconnect
  - _trigger_reconnect is a no-op if _reconnecting is True
